...
Application **str-perm-gen-b** has been designed to dry run the permutation engine for benchmarking purposes. Given an input string the appication prints summary level information about how many permutations have been generated, how many test cycles have been executed and what the average execution time per test cycle is. Only the default permutation engine is benchmarked. The lexicographic permutation engine is not benchmarked.
 ...
 * **str_perm_gen_b.cpp** - this is the main entry module of the str-perm-gen-b application. It implements a string type specialisation of the generic permutation engine along with a dedicated command line interface. The purpose is to benchmark performance of the string permutation process;
 * **str_perm_gen_main.cpp** - the main entry module of the str-perm-gen application. It uses a dedicated command line interface to drive a string type specialisation of the generic permutation engine. The purpose is to provide a command line tool that efficiently delivers string permutations;
 * **str_perm_gen.hpp** - declares the string type specialisation of the permutation engine for the str-perm-gen application;
 * **str_perm_gen.cpp** - defines the string type specialisation of the permutation engine for the str-perm-gen application;
