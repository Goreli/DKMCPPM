...
This directory contains C++ source code written to address the String Permutation Generator challenge discussed at the Melbourne C++ Meetup Olympiad on 12/Nov/2019. A reusable C++ template header file has been developed that allows to calculate permutations of arbitrary trivial type sequences. Also, two applications have been developed that specialise the permutation generator specifically for the purpose of string permutation and provide command line interfaces to drive their respective functions.

Application **str-perm-gen-b** has been designed to dry run the permutation generator for benchmarking purposes. Given an input string the appication prints summary level information about how many permutations have been generated, how many test cycles have been executed and what the average execution time per test cycle is. Only the default permutation generator is benchmarked with the duplicate permutations mode enabled. The lexicographic permutation generator is not benchmarked.

Application **str-perm-gen** prints actual permutations of the input string specified. It may take long time to print all possible permutations of a long input string. The application accepts command line arguments that help limit the number of permutations generated in order to keep the execution time below a reasonable limit. Also, in cases where it takes long time to print the users should be able to impove performance of the program by redirecting the standard output to a disk file. 
...
![alt text](data/sp_example.png "Example output of str-perm-gen-b and str-perm-gen.")
 ...
 * **str_perm_gen_b.cpp** - this is the main entry module of the str-perm-gen-b application. It implements a string type specialisation of the generic permutation generator along with a dedicated command line interface. The purpose is to benchmark performance of the string permutation process;
 ...
 * **str_perm_gen_cli.hpp** - declares the command line parser class for the str-perm-gen application;
 ...
 * **-d**  - allows the permutation generator to generate duplicate permutations. By default duplicate permutations are not allowed. This option is not compatible with the **-l** option;
 ...
 * **-r gsize**  - randomly pick one permutation from each consecutive group generated by the default permutation generator. The gsize number specifies the size of the groups. This option invalidates options **-d**, **-l** and **-p**.
...
8. Robert Sedgewick, Permutation Generation Methods, Computing Surveys, Vol 9, No 2, June 1977
