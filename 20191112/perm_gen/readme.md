...
Application **str-perm-gen** prints actual permutations of the input string specified. It may take long time to print all possible permutations of a long input string. The application accepts a command line argument that limits the number of permutations generated in order to keep the execution time below a reasonable limit. Also, in cases where it takes long time to print the output on the screen the users should be able to impove performance of the program by redirecting the standard output to a disk file. 
