...
The [2d_convex_hull](https://github.com/Goreli/DKMCPPM/tree/master/20191112/2d_convex_hull) directory also contains a Linux shell script called gen-inp-file2d.sh that generates input files containing random data for the solution. The x and y values generated by the script vary in the [-1.0, +1.0] range. The script depends on presence of the awk tool. Refer the content of Makefile to see example command line syntax. Be aware that the script uses a random number generator that creates a different data file every time it runs. Should there be a need to preserve an older data file it should be backed up at a different location to preclude the destruction of the dataset.
